...
export class SelectMapButton extends MapButton {
    ...
    open() {
        ...
        this.parent.geometryHandler.activate();
        this._open = true;
    }

    close() {
        this.button.classList.remove("active");
        this.parent.geometryHandler.deactivate();
        this._open = false;
    }
}
...
export class LayersButton extends MapButton {
    ...
    initListeners() {
        ...
        const workspacesList = this._layersMenu.querySelector(".cgp-layers__list");
            ...
            item.innerHTML = `
                <div class="item-header">
                    ...
                    <div class="item-actions">
                        ...
                        <button class="cgp-map_button small">${SearchIcon}</button>
                    </div>
                </div>
                <div class="item-content">
                    <span>ID: ${workspace.id}</span>
                    <div class="item-content__info"><p>Статус:</p><span>${workspace.status}</span></div>
                    <div class="item-content__info"><p>Время обработки:</p><span>${workspace.elapsed}</span></div>
                    <div class="item-content__info"><p>Изображений:</p><span>${workspace.images}</span></div>
                    <div class="item-content__info"><p>Прогресс:</p><span>${workspace.progress}%</span></div>
                    ${
                        workspace.error
                            ? `<div class="item-content__info"><p>Ошибка:</p><span>${workspace.error}</span></div>`
                            : ""
                    }
                </div>
            `;
