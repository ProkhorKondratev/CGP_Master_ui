...
class WorkNode {
    ...
    initListeners() {
        ...
        editButtons[0].onclick = () => this.deleteNode();
    ...
    showTasks() {
        ...
        tasksList.className = "wn-tasks";

        if (this.tasks.length === 0) {
            tasksList.innerHTML = `<p style="text-align: center">Задачи отсутствуют</p>`;
            this.parent.modal.open("Задачи узла", tasksList);
            return;
        }

        this.tasks.forEach((task) => {
            const taskElement = document.createElement("div");
            taskElement.className = "wn-tasks__item";
            taskElement.innerHTML = `
                <div class="item-header">
                    <span>Название: ${task.name}</span>
                    <button class="wn__button modal">${TrashIcon}</button>
                </div>
                <div class="item-content">
                    <div class="item-content__info">
                        <div class="info"><p>Статус:</p><span>${task.status}</span></div>
                        <div class="info"><p>Изображений:</p><span>${task.images}</span></div>
                        <div class="info"><p>Время:</p><span>${task.elapsed}</span></div>
                        <div class="info"><p>Идентификатор:</p><span>${task.id}</span></div>
                    </div>
                </div>
            `;

            tasksList.appendChild(taskElement);
            taskElement.querySelector("button").onclick = () => {
